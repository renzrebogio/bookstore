...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif ($stock === '' || !is_numeric($stock) || intval($stock) < 0) {
        ...
        $image_file_name = '../../src/placeholder.jpg'; 
        $upload_success = true;
        ...
        if (isset($_FILES['product_image']) && $_FILES['product_image']['error'] == 0) {
            ...
            if (move_uploaded_file($_FILES['product_image']['tmp_name'], $final_destination)) {
                ...
                $msg_text = "Upload Error: Hindi maigalaw ang file sa folder.";
            }
        }
        ...
        if ($upload_success) {
            try {
                ...
                $query = "INSERT INTO products (category_id, product_name, description, price, stock_quantity, product_image, status) 
                          VALUES (?, ?, ?, ?, ?, ?, ?)";
